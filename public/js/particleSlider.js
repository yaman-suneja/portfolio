function ParticleSlider(a) {
    ...
    (b.$container = b.$("#" + b.sliderId)),
        ...
        (b.$controlLeft = null),
        (b.$controlRight = null),
        (b.$canv = b.$(".yaman-suneja-draw-particles")),
...
(psParticle.prototype.move = function () {
    ...
    if (this.ttl != null && this.ttl-- <= 0) a.swapList(b, a.pxlBuffer, a.recycleBuffer), (this.ttl = null);
        ...
        if (a.mx >= 0 && a.mouseForce) {
            var j = this.x - a.mx,
                k = this.y - a.my;
            (h = Math.min(a.mouseForce / (Math.pow(j, 2) + Math.pow(k, 2)), a.mouseForce)), (i = Math.atan2(k, j)), typeof this.color == "function" && ((i += Math.PI), (h *= 0.001 + Math.random() * 0.1 - 0.05));
        } else (h = 0), (i = 0);
        (this.velocityX += g * Math.cos(f) + h * Math.cos(i)), (this.velocityY += g * Math.sin(f) + h * Math.sin(i)), (this.velocityX *= 0.92), (this.velocityY *= 0.92), (this.x += this.velocityX), (this.y += this.velocityY);
    }
}),
